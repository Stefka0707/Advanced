...
public class MatrixShuffling {
    public static void main(String[] args) {
        ...
        fillStringMatrix(matrix, scanner);
        String command = scanner.nextLine();

        while (!command.equals("END")) {
            //валидиране на комада
            if (validateCommand(command, rows, cols)) {
                String[] commandParts = command.split("\\s+");
                int row1 = Integer.parseInt(commandParts[1]);// валиден е : row >=0 && row < броя на редовете
                int col1 = Integer.parseInt(commandParts[2]);// валиден е : col >=0 && col < броя на колоните
    ...
    private static boolean validateCommand(String command, int rows, int cols) {
        // true -> ако командата е валидни
        // false -> ако командата е Невалидна
        // броя на частите в командата да е 5
        String[] commandParts = command.split("\\s+");
